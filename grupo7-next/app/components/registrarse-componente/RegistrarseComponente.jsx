...
function RegistrarseComponente() {
	...
	const handleSubmit = async (e) => {
		...
		if (password !== confirmPassword) {
		   toast.error("Las contraseñas no coinciden. Inténtalo de nuevo.", config);
			setPassword("")
			setConfirmPassword("")
			return;
		 } 
		...
		try {
		  const response = await fetch(`${process.env.NEXT_PUBLIC_APIURL}/api/auth/local/register`, {
			 method: 'POST',
			 headers: {
          ...
          if(errorMessage.includes('Email')) {
            toast.error("Usuario ya registrado en Grupo 7, por favor inicia sesión")
				setUsuario("")
				setEmail("")
				setPassword("")
				setConfirmPassword("")
            ...
            toast.error("Error desconocido, por favor inténtalo otra vez")
				setLoading(false)
          }
		  }
		...
		<div className={styles.contenedor}>
			...
			<div className={styles.divForm}>
				<Form className={styles.form} onSubmit={handleSubmit}>
					...
					<Button variant="primary" type="submit" className={`${styles.button} w-full`}>
						REGISTRARSE
					</Button>
