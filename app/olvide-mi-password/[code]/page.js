...
function Code({ params }) {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (data.password !== data.passwordConfirmation) {
				alert("Las contraseñas ingresadas deben ser idénticas", config);
				setTimeout(() => {
					setLoading(false);
				}, 3000);
				return;
			...
			let response = await fetch(
				`${process.env.NEXT_PUBLIC_APIURL}/api/auth/reset-password`,
				{
					method: "POST",
					headers: {
			...
			if (response.ok) {
				...
				if (errorMessage.includes("code")) {
					alert(
						"Hey!, no es bueno meterse con las contraseñas ajenas!",
						config
					);
					setTimeout(() => {
						setLoading(false);
	...
	return (
		<div className={styles.container}>
			<h1 className={styles.pageH1}>Reestableciendo mi Contraseña</h1>
